...
public class BouncySSLUtil extends AbstractSSLUtilBase {

    private static final Log log = LogFactory.getLog(BouncySSLUtil.class);
    private static final StringManager sm = StringManager.getManager(BouncySSLUtil.class);
    ...
    static {
        SSLContext context;
        try {
            context = new ExposeJSSESSLContext(Constants.SSL_PROTO_TLS, new BouncyCastleJsseProvider());
    ...
    public BouncySSLUtil(SSLHostConfigCertificate certificate) {
        super(certificate);
    }
    ...
    public SSLContext createSSLContext(List<String> negotiableProtocols) throws NoSuchAlgorithmException {
        return new ExposeJSSESSLContext(sslHostConfig.getSslProtocol(), new BouncyCastleJsseProvider());
    }
